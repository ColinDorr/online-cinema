<template>
  ...
  <div v-else class="grid px-10">
    ...
    <p
      class="mx-auto mb-3 mt-10 text-lg text-white md:mb-8 md:mt-20 md:text-5xl md:font-semibold"
    >
      Who's Watching?
    </p>
    ...
    <div class="mx-auto flex flex-col lg:flex-row flex-wrap gap-5">
      <ul class="flex flex-row flex-wrap gap-5">
        ...
        >
          <button
            @click="handleAvatarClick(userKey)"
            :aria-label="`select ${userKey} avatar`"
            class
          >
            <User
