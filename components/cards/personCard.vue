<template>
  ...
  >
    <NuxtLink :to="`/people/${data.id}`" class="w-full h-full">
        <imageElement :path="data.image" :alt="data.name" :placeholder="placeholder" class="w-full h-full"/>
    </NuxtLink>

    <div class="hidden md:block absolute bottom-0 left-0 w-full bg-black p-4 text-center">
...
import placeholder from "@/src/assets/person.webp";
