...
const listedMovies = computed(() => {
  return streamingData.movies.filter((item) =>
    listedMoviesIds.value.includes(item.id),
  );
});

const listedSeries = computed(() => {
  return streamingData.series.filter((item) =>
    listedSeriesIds.value.includes(item.id),
  );
});
