<template>
  <div class="container mx-auto px-5 lg:px-0 !py-20">
    <div class="flex flex-col gap-10">
      ...
      <div class="flex flex-wrap gap-5 justify-center lg:grid lg:grid-cols-5 xl:grid-cols-6">
        <div v-for="person in displayPeople" :key="person.id" class="w-[200px] lg:w-auto">
          <PersonCard :path="'person'" :data="person" class="w-[200px] lg:w-auto"/>
        </div>        
      </div>
    </div>
  </div>
...
const loadMore = () => {
  ...
  const nextPeople = streamingData.people.slice(currentLength, currentLength + 50)
